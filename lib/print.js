...
function textStream (appName, appDir, filetype, contents) {
  return new StringStream(getText(appName, appDir, filetype, contents) + '\n')
}

function printAny (appName, appDir, filetype, contents) {
  if (typeof contents === 'function')
    contents = contents()
  if (typeof contents === 'object' && contents.pipe) {
    return contents
      .pipe(split())
      .pipe(through(printText.bind(null, appName, appDir, filetype)))
  } else {
    return textStream(appName, appDir, filetype, contents)
  }
}
...
function localisedFileStream (appName, appDir, file, lang) {
  ...
  return file ? createFileStream(appName, appDir, file) : new StringStream('')
}
