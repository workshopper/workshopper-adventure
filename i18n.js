...
function chooseLang (globalStorage, appStorage, defaultLang, availableLangs, lang) {
  ...
  const data = ((appStorage && appStorage.get('lang')) || globalStorage.get('lang') || {})
...
exports.init = (options, globalStorage, appStorage) => {
  const lookup = i18nChain(
    options.appDir ? i18nFs(path.resolve(options.appDir, './i18n')) : null
    , i18nFs(path.resolve(__dirname, './i18n'))
  )
  const root = i18n(lookup)
  root.fallback = key => `?${key}?`
  const choose = chooseLang.bind(null, globalStorage, appStorage, options.defaultLang, options.languages)
  let lang = choose(null)
  const translator = root.section(lang, true)
  // TODO: _excercises is unused... is this ok?
  // eslint-disable-next-line
  const result = i18n(i18nExtend(translator, key => {
    if (options[key]) {
      return options[key]
    }
  ...
  return {
    ...
    extend: obj => {
      return i18n(i18nExtend(result, key => obj[key]))
    },
    lang: () => lang
  }
}
